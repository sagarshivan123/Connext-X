    ...
    <div className="flex flex-col flex-1 bg-slate-900 text-white">
      ...
      <header className="p-4 bg-slate-800 border-b border-slate-700 flex justify-between items-center shadow-md">
        <div className="flex items-center gap-3">
          <div className="relative group">
            <img src={avatarSrc} className="w-12 h-12 rounded-full object-cover" alt="chat-avatar" />
            {isGroupAdmin && (
              <button onClick={() => groupImageInputRef.current.click()} className="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                📷
              </button>
            )}
          </div>
          <div>
            <h2 className="font-bold">{chatType === "private" ? activeFriend?.name : activeGroup?.name}</h2>
            <p className="text-xs text-slate-400">{chatType === "group" && `${activeGroup?.members?.length} members`}</p>
          </div>
        </div>
        ...
        {messages?.map((msg, i) => {
          ...
          return (
            <div key={msg._id || i} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
              <div className={`max-w-[75%] p-3 rounded-2xl ${isMe ? 'bg-blue-600' : 'bg-slate-800 border border-slate-700'}`}>
                  ...
                  <div className="mt-2 overflow-hidden rounded-lg">
                    {msg.mediaType === "video" ? (
                      ...
                      <img 
                        src={mediaUrl} 
                        alt="Shared media" 
                        className="max-h-60 w-full object-cover cursor-pointer hover:opacity-90"
      ...
      {ui.addModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-slate-800 p-6 rounded-2xl w-80 shadow-xl border border-slate-700">
            <h3 className="font-bold mb-4">Add Member</h3>
            <input className="w-full p-2 bg-slate-700 rounded border border-slate-600 mb-4 outline-none" placeholder="User Email" value={memberEmail} onChange={e => setMemberEmail(e.target.value)} />
            <div className="flex gap-2">
              <button className="flex-1 text-slate-400" onClick={() => setUi({...ui, addModal: false})}>Cancel</button>
              <button className="flex-1 bg-blue-600 py-2 rounded-lg" onClick={() => handleMemberAction('add')}>Add</button>
            </div>
          </div>
        </div>
      )}
      ...
      {ui.listModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-slate-800 p-6 rounded-2xl w-96 max-h-[80vh] flex flex-col border border-slate-700">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold">Members</h3>
              <button onClick={() => setUi({...ui, listModal: false})}>✕</button>
            </div>
            <div className="overflow-y-auto space-y-3">
              {activeGroup?.members?.map(m => (
                <div key={m._id} className="flex items-center justify-between p-2 bg-slate-700/30 rounded-lg">
                  <div className="flex items-center gap-3">
                    <img src={m.avatar || `https://ui-avatars.com/api/?name=${m.name}`} className="w-8 h-8 rounded-full" />
                    <span className="text-sm">{m.name} {m._id === user._id && "(You)"}</span>
                  </div>
                  {isGroupAdmin && m._id !== user._id && (
                    <button onClick={() => handleMemberAction('remove', m._id)} className="text-red-400 text-xs hover:underline">Remove</button>
                  )}
                </div>
              ))}
            </div>
